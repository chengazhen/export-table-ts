var exportData = (() => {
  ...
  var ExprotExcel = class {
    ...
    buildExcelRow(mainKeys, headers, rawDataList) {
      ...
      for (const rawData of rawDataList) {
        ...
        for (const header of headers) {
          if (header.prop && rawData["rowSpan"] === 0 && mainKeys.has(header.prop)) {
    ...
    flat(revealList, d = 1) {
      const result = [];
      return d > 0 ? revealList.reduce((acc, val) => acc.concat(Array.isArray(val.child) ? this.flat(val.child, d - 1) : val), result) : revealList.slice();
    }
